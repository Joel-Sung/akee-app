...
export default function TransactionsAndLiquidityChart(
  props: TransactionsAndLiquidityChartProps
) {
  const { cid, initialRange = "7d" } = props;
  ...
  useEffect(() => {
    async function fetchData() {
      const request = await fetch(
        `${process.env.NEXT_PUBLIC_MY_URL}/api/transactionsAndLiquidity?cid=${cid}&range=${chartRange}`
      );
      await request.json().then((responseJSON) => {
        setTransfersLabels(
          msArrayToDateTimeStringArray(responseJSON.data.transfers.values.x)
        );
        setTransfersData(responseJSON.data.transfers.values.y);
        setLiquidityData(
          responseJSON.data.liquidity.values.y.map((value: number | null) =>
  ...
  return (
    <ComponentContainer>
      ...
      <ComponentInfo>
        <ValueCard
          title={"Sales(" + printTimeRange(chartRange) + ")"}
          value={sales}
          delta={salesDelta}
        />
        <ValueCard
          title={"Transfers(" + printTimeRange(chartRange) + ")"}
          value={transfers}
          delta={transfersDelta}
          isShorten={true}
        />
        <ValueCard
          title={"Liquidity(" + printTimeRange(chartRange) + ")"}
          value={liquidity}
