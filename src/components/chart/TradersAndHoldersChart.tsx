...
export default function TradersAndHoldersChart(
  props: TradersAndHoldersChartProps
) {
  ...
  useEffect(() => {
    async function fetchData() {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_MY_URL}/api/tradersAndHolders?cid=${cid}&range=${chartRange}`
      );
      await response.json().then((responseJSON) => {
        setBuyersLabels(
          responseJSON.data.buyers.values.x.map((x: number | null) => {
  ...
  return (
    <ComponentContainer>
      ...
      <ComponentInfo>
        <ValueCard
          title={"Buyers(" + printTimeRange(chartRange) + ")"}
          value={buyers}
          delta={buyersDelta}
        />
        <ValueCard
          title={"Sellers(" + printTimeRange(chartRange) + ")"}
          value={sellers}
          delta={sellersDelta}
        />
        <ValueCard
